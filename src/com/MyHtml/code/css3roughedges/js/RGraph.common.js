...
RGraph.Tooltip = function(canvas, text, x, y)
{
    ...
    if (placementLeft && effect == 'fade')
    {
        obj.style.top = (y - height) + 'px';
    }
    else
    if (effect != 'fade' && effect != 'expand')
    {
...
RGraph.DrawKey = function(obj, key, colors)
{
    ...
    if (keypos && keypos == 'gutter')
    {
        ...
        if (obj.type == 'pie' && obj.Get('chart.align') == 'right')
        {
        ...
        for ( i = 0; i < key.length; ++i)
        {
            ...
            RGraph.Text(context, obj.Get('chart.text.font'), textsize, start + 25, gutter - 6 - textsize, key[i], 'top');
            context.fill();
...
RGraph.Annotate = function(obj)
{
    if (obj.Get('chart.annotatable'))
    {
        ...
        canvas.onmousemove = function(e)
        {
            ...
            if (x > gutter && x < (width - gutter) && y > gutter && y < (height - gutter))
            {
...
RGraph.Zoom = function(e)
{
    ...
    setTimeout(function()
    {
        window.onclick = __HideZoomedCanvas__;
    }, 1);
    ...
    if (obj.Get('chart.zoom.hdir') == 'left')
    {
        for (var i = 1; i <= frames; ++i)
        {
            var newWidth = width * factor * (i / frames) + width;
    ...
    if (obj.Get('chart.zoom.vdir') == 'center')
    {
        for (var i = 1; i <= frames; ++i)
        {
            var newHeight = (height * factor * (i / frames)) + height;
