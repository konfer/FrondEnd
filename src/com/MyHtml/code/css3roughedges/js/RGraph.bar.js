...
{
    if ((this.Get('chart.variant') == 'pyramid' || this.Get('chart.variant') == 'dot') && typeof (this.Get('chart.tooltips')) == 'object' && this.Get('chart.tooltips') && this.Get('chart.tooltips').length > 0)
    {
...
RGraph.Bar.prototype.DrawAxes = function()
{
    ...
    for ( y = gutter; this.Get('chart.xaxispos') == 'center' ? y <= (this.canvas.height - gutter) : y < (this.canvas.height - gutter); y += yTickGap)
    {
        if (this.Get('chart.xaxispos') == 'center' && y == (this.canvas.height / 2))
            continue;
...
RGraph.Bar.prototype.Drawbars = function()
{
    ...
    if (this.Get('chart.ymax'))
    {
        ...
        this.scale = [(this.max * (1 / 5)).toFixed(this.Get('chart.scale.decimals')), (this.max * (2 / 5)).toFixed(this.Get('chart.scale.decimals')), (this.max * (3 / 5)).toFixed(this.Get('chart.scale.decimals')), (this.max * (4 / 5)).toFixed(this.Get('chart.scale.decimals')), this.max.toFixed(this.Get('chart.scale.decimals'))];
    }
    else
    {
    ...
    for ( i = 0; i < this.data.length; ++i)
    {
        ...
        if ( typeof (this.data[i]) == 'object' && this.Get('chart.grouping') == 'stacked')
        {
            var barWidth = width - (2 * hmargin);
            var redrawCoords = [];
            var startY = 0;
        ...
        if ( typeof (this.data[i]) == 'object' && this.Get('chart.grouping') == 'grouped')
        {
            for ( j = 0; j < this.data[i].length; ++j)
            {
                this.context.strokeStyle = this.Get('chart.strokecolor');
    ...
    if (this.Get('chart.tooltips'))
    {
        ...
        this.canvas.onmousemove = function(e)
        {
            ...
            for (var i = 0; i < obj.coords.length; i++)
            {
                ...
                if (mouseX >= (left + 5  ) && mouseX <= (left + width - 5) && mouseY >= top && mouseY <= (top + height))
                {
...
RGraph.Bar.prototype.DrawLabels = function()
{
    ...
    if ( typeof (labels) == 'object' && labels)
    {
        ...
        {
            RGraph.Text(context, this.Get('chart.text.font'), text_size, x, (this.canvas.height - gutter) + yOffset, String(labels[i++]), null, halign, null, angle);
        }
    }
};
